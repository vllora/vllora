---
description: |
  dry-run on 4B + 0.8B; readiness gate; re-run to iterate.
  Triggers: "eval readiness", "check learnable", "run eval".
  Preconditions: generate done, quality_gate=pass.
allowed-tools: Bash
---
...
## When to use
dry-run on 4B + 0.8B; readiness gate; re-run to iterate. Preconditions: generate done, quality_gate=pass.

## Steps
1. Verify preconditions via `vllora finetune status`.
2. Shell out: `vllora finetune eval`
