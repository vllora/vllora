...
fn load_working_directory_agents(work_dir: &Path) -> Result<HashMap<String, AgentDefinition>, AgentError> {
    ...
    if !agents_dir.is_dir() {
        warn!("agents path exists but is not a directory: {:?}", agents_dir);
        return Ok(agents);
    }
...
fn merge_agents(
    ...
    for (name, agent) in working_dir {
        info!("Overriding embedded agent with working directory version: {}", name);
...
pub async fn register_agents() -> Result<(), AgentError> {
    ...
    if !check_distri_running(&api_url).await {
        eprintln!("⚠️  Distri server is not running or not reachable at {}", api_url);
        eprintln!("   Skipping agent registration. Start Distri server to register agents.");
        warn!("Distri server is not running or not reachable at {}", api_url);
        return Ok(());
    }
    ...
    info!("Loaded {} agents from working directory", working_dir_agents.len());
    ...
    if error_count > 0 {
        ...
        eprintln!("⚠️  Registered {}/{} agents successfully. {} failed.", success_count, agents.len(), error_count);
    } else {
