...
pub struct ResponsesResultStream {
    inner: Pin<Box<dyn Stream<Item = Result<ResponseStreamEvent, LLMError>> + Send + 'static>>,
}
...
impl ResponsesResultStream {
    ...
    pub fn create(
        rx: tokio::sync::mpsc::Receiver<Result<ResponseStreamEvent, LLMError>>,
...
impl Stream for ResponsesResultStream {
    type Item = Result<ResponseStreamEvent, LLMError>;
